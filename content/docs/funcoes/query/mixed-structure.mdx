...
Como foi visto na estrutura básica e no capítulo de [Cláusulas Linguísticas](./base-structure.mdx), a consulta dentro da função query deve ser uma string, a qual será passada entre aspas duplas ("consulta"). Essa característica não impede que a consulta seja o resultado de uma função, desde que esse resultado siga as características que toda consulta deve ter, como em:

```text
=QUERY(dados;IF(A1 = "valor de A1";"select A, B where C = 'opção 1'";"select A,B where C = 'Opção 2'"); [cabeçalhos])
```

Ainda pode ser utilizado da mesma lógica para juntar duas ou mais strings para gerar uma consulta, como em:
