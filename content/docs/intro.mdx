---
id: introducao-planilhas
title: Introdução a Planilhas
sidebar_label: Introdução
slug: /introducao-planilhas
sidebar_position: 0
description: Motivação para aprender planilhas (Google Sheets), com links úteis de documentação e PBL.
---

# Introdução a Planilhas

> Como produzir mais fácil e rápido (de graça)

## Motivador

Quando falamos do cotidiano profissional e acadêmico — e foi no ambiente militar onde aprendi isso de forma mais clara — uma das maiores **guerras** que travamos é contra o **tempo**. Por isso, vale-se aprender **meios** de **aumentar** nossa **eficiência** e consequentemente **melhorar** nossa **qualidade de vida**. Mas Por que ?

$$
 \text{Eficiência} = {\text{Trabalho} \over \text{Tempo Gasto}}
$$

## Onde Aprender

Um dos principais pontos que precisam ser entendidos para qualquer assunto é **como continuar o aprendizado**, principalmente para além das aulas.

- ### Documentação

  Um dos melhores **ambientes de estudo** em todo e qualquer sistema, framework ou bibliotecas é a **página de documentação**, e para a maioria das formulas a documentação do Google Sheets consegue suprir todas necessidades.

  Links úteis:

  - [Documentação da Google](https://support.google.com/docs)
  - [Documentação do Sheets](https://support.google.com/docs/topic/9054603)
  - [Lista de Funções](https://support.google.com/docs/table/25273)

- ### Livros

  Para o lado de **análise de dados** — o “por que” e o “para quê” por trás das fórmulas —, a referência recomendada é:

  - **PROVOST, Foster; FAWCETT, Tom.** *Data Science for Business*. O’Reilly, 2013 — de onde vêm os [dois tipos de decisão orientada por dados](/docs/analise-de-dados) discutidos neste handbook.

- ### PBL

  **Project Based Learning** (**PBL**) é uma das **técnicas** mais **conhecidas** e **efetivas** para **aprender**. Basicamente consiste em iniciar **projetos** para colocar em **prática** os conhecimentos que querem ser adquiridos.

  > Para isso foi criada toda a seção de [Prática](./Práticas/_category_.json) e [Estudos de Caso](./Casos%20de%20Estudo/_category_.json)
